...
func clusterConnectionIsOpen(cluster RedisCluster) bool {
	...
	if err := c.Set(context.Background(), testKey, "test", time.Second).Err(); err != nil {
		log.Warnf("Error trying to set test key: %s", err.Error())

		return false
	}
	if _, err := c.Get(context.Background(), testKey).Result(); err != nil {
...
func (r *RedisCluster) GetKey(keyName string) (string, error) {
	...
	value, err := cluster.Get(context.Background(), r.fixKey(keyName)).Result()
	if err != nil {
...
func (r *RedisCluster) GetMultiKey(keys []string) ([]string, error) {
	...
	case *redis.ClusterClient:
		{
			...
			for _, key := range keyNames {
				getCmds = append(getCmds, pipe.Get(context.Background(), key))
			}
	...
	case *redis.Client:
		{
			values, err := cluster.MGet(context.Background(), keyNames...).Result()
			if err != nil {
				log.Debugf("Error trying to get value: %s", err.Error())
...
func (r *RedisCluster) GetKeyTTL(keyName string) (ttl int64, err error) {
	...
	duration, err := r.singleton().TTL(context.Background(), r.fixKey(keyName)).Result()
...
func (r *RedisCluster) GetRawKey(keyName string) (string, error) {
	...
	value, err := r.singleton().Get(context.Background(), keyName).Result()
	if err != nil {
		log.Debugf("Error trying to get value: %s", err.Error())
...
func (r *RedisCluster) GetExp(keyName string) (int64, error) {
	...
	value, err := r.singleton().TTL(context.Background(), r.fixKey(keyName)).Result()
	if err != nil {
...
func (r *RedisCluster) SetExp(keyName string, timeout time.Duration) error {
	...
	err := r.singleton().Expire(context.Background(), r.fixKey(keyName), timeout).Err()
	if err != nil {
...
func (r *RedisCluster) SetKey(keyName, session string, timeout time.Duration) error {
	...
	err := r.singleton().Set(context.Background(), r.fixKey(keyName), session, timeout).Err()
	if err != nil {
...
func (r *RedisCluster) SetRawKey(keyName, session string, timeout time.Duration) error {
	...
	err := r.singleton().Set(context.Background(), keyName, session, timeout).Err()
	if err != nil {
		log.Errorf("Error trying to set value: %s", err.Error())
...
func (r *RedisCluster) Decrement(keyName string) {
	...
	err := r.singleton().Decr(context.Background(), keyName).Err()
	if err != nil {
...
func (r *RedisCluster) IncrememntWithExpire(keyName string, expire int64) int64 {
	...
	fixedKey := keyName
	val, err := r.singleton().Incr(context.Background(), fixedKey).Result()
	...
	if val == 1 && expire > 0 {
		...
		r.singleton().Expire(context.Background(), fixedKey, time.Duration(expire)*time.Second)
	}
...
func (r *RedisCluster) GetKeys(filter string) []string {
	...
	fnFetchKeys := func(client *redis.Client) ([]string, error) {
		...
		iter := client.Scan(context.Background(), 0, searchStr, 0).Iterator()
	...
	case *redis.ClusterClient:
		...
		go func() {
			err = v.ForEachMaster(context.Background(), func(ctx context.Context, client *redis.Client) error {
...
func (r *RedisCluster) GetKeysAndValuesWithFilter(filter string) map[string]string {
	...
	case *redis.Client:
		{
			result, err := v.MGet(context.Background(), keys...).Result()
			if err != nil {
				log.Errorf("Error trying to get client keys: %s", err.Error())
...
func (r *RedisCluster) DeleteKey(keyName string) bool {
	...
	n, err := r.singleton().Del(context.Background(), r.fixKey(keyName)).Result()
	if err != nil {
...
func (r *RedisCluster) DeleteAllKeys() bool {
	...
	n, err := r.singleton().FlushAll(context.Background()).Result()
	if err != nil {
...
func (r *RedisCluster) DeleteRawKey(keyName string) bool {
	...
	n, err := r.singleton().Del(context.Background(), keyName).Result()
	if err != nil {
		log.Errorf("Error trying to delete key: %s", err.Error())
	}
...
func (r *RedisCluster) DeleteKeys(keys []string) bool {
	...
	if len(keys) > 0 {
		...
		case *redis.Client:
			{
				_, err := v.Del(context.Background(), keys...).Result()
				if err != nil {
					log.Errorf("Error trying to delete keys: %s", err.Error())
				}
			}
...
func (r *RedisCluster) Publish(channel, message string) error {
	...
	err := r.singleton().Publish(context.Background(), channel, message).Err()
	if err != nil {
		log.Errorf("Error trying to set value: %s", err.Error())
...
func (r *RedisCluster) AppendToSet(keyName, value string) {
	...
	if err := r.singleton().RPush(context.Background(), fixedKey, value).Err(); err != nil {
...
func (r *RedisCluster) Exists(keyName string) (bool, error) {
	...
	exists, err := r.singleton().Exists(context.Background(), fixedKey).Result()
	if err != nil {
...
func (r *RedisCluster) RemoveFromList(keyName, value string) error {
	...
	if err := r.singleton().LRem(context.Background(), fixedKey, 0, value).Err(); err != nil {
		log.Error(
			"LREM command failed",
			log.String("keyName", keyName),
			log.String("fixedKey", fixedKey),
...
func (r *RedisCluster) GetListRange(keyName string, from, to int64) ([]string, error) {
	...
	elements, err := r.singleton().LRange(context.Background(), fixedKey, from, to).Result()
	if err != nil {
		log.Error(
...
func (r *RedisCluster) AppendToSetPipelined(key string, values [][]byte) {
	...
	for _, val := range values {
		pipe.RPush(context.Background(), fixedKey, val)
	}
...
func (r *RedisCluster) GetSet(keyName string) (map[string]string, error) {
	...
	val, err := r.singleton().SMembers(context.Background(), r.fixKey(keyName)).Result()
	if err != nil {
...
func (r *RedisCluster) AddToSet(keyName, value string) {
	...
	err := r.singleton().SAdd(context.Background(), r.fixKey(keyName), value).Err()
	if err != nil {
...
func (r *RedisCluster) RemoveFromSet(keyName, value string) {
	...
	err := r.singleton().SRem(context.Background(), r.fixKey(keyName), value).Err()
	if err != nil {
...
func (r *RedisCluster) IsMemberOfSet(keyName, value string) bool {
	...
	val, err := r.singleton().SIsMember(context.Background(), r.fixKey(keyName), value).Result()
	if err != nil {
...
func (r *RedisCluster) SetRollingWindow(
	...
	pipeFn := func(pipe redis.Pipeliner) error {
		pipe.ZRemRangeByScore(context.Background(), keyName, "-inf", strconv.Itoa(int(onePeriodAgo.UnixNano())))
		zrange = pipe.ZRange(context.Background(), keyName, 0, -1)
		...
		pipe.ZAdd(context.Background(), keyName, element)
		pipe.Expire(context.Background(), keyName, time.Duration(per)*time.Second)
	...
	var err error
	if pipeline {
		_, err = client.Pipelined(context.Background(), pipeFn)
	} else {
		_, err = client.TxPipelined(context.Background(), pipeFn)
	}
...
func (r *RedisCluster) AddToSortedSet(keyName, value string, score float64) {
	...
	if err := r.singleton().ZAdd(context.Background(), fixedKey, member).Err(); err != nil {
		log.Error(
			"ZADD command failed",
			log.String("keyName", keyName),
			log.String("fixedKey", fixedKey),
...
func (r *RedisCluster) GetSortedSetRange(keyName, scoreFrom, scoreTo string) ([]string, []float64, error) {
	...
	values, err := r.singleton().ZRangeByScoreWithScores(context.Background(), fixedKey, &args).Result()
	if err != nil {
		log.Error(
...
func (r *RedisCluster) RemoveSortedSetRange(keyName, scoreFrom, scoreTo string) error {
	...
	if err := r.singleton().ZRemRangeByScore(context.Background(), fixedKey, scoreFrom, scoreTo).Err(); err != nil {
		log.Debug(
